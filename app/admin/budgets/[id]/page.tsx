...
async function BudgetDetailsPage({ params }: { params: { id: string } }) {
  const budget = await getBudgetById(params.id)
  ...
  const percentageSpent = +((totalSpent / +budget.amount) * 100).toFixed(2)
    ...
    <>
      ...
      {budget.expenses.length ? (
        ...
        <p className="text-gray-600 mt-10 text-center">
          No hay gastos registrados para este presupuesto. ¡Agrega tu primer
          gasto!
        </p>
      )}
      <ModalContainer />
