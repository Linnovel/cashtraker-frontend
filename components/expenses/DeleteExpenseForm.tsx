...
import { deleteActionExpense } from "@/actions/deliete-expense-action"
...
export default function DeleteExpenseForm({ closeModal }: DeleteExpenseForm) {
  ...
  const deleteExpenseId = deleteActionExpense.bind(null, {
    budgetId: +budgetId,
    expenseId: +expenseId,
  })

  const [state, dispatch] = useFormState(deleteExpenseId, {
    error: [],
    success: "",
  })

  //Si el id del presupuesto o del gasto no es un numero entero, se cierra el modal
  useEffect(() => {
  ...
  return (
    <>
      ...
      <form action={dispatch} className="grid grid-cols-2 gap-5 mt-10">
        <button
